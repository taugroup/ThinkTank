...
class GeminiProWrapper(DeepEvalBaseLLM):
    ...
    def generate(self, prompt: str, schema: Optional[BaseModel] = None, **kwargs) -> Union[str, BaseModel]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # If schema is provided, try to parse structured response
                if schema is not None:
                    # Special handling for DeepEval's Steps schema
                    if schema == Steps:
                        return Steps(steps=[response_text])
                    return self._generate_with_schema_response(response_text, schema)
                
                # Check if this is being called by DeepEval (look for evaluation keywords in prompt)
                if any(keyword in prompt.lower() for keyword in ['evaluate', 'score', 'rate', 'assess', 'criteria']):
                    # Return Steps object for DeepEval compatibility
                    return Steps(steps=[response_text])
                ...
                # Return Steps object even for errors when it looks like evaluation
                if any(keyword in prompt.lower() for keyword in ['evaluate', 'score', 'rate', 'assess', 'criteria']):
                    return Steps(steps=[f"Error: {str(e)}"])
                return ""
    ...
    async def a_generate(self, prompt: str, schema: Optional[BaseModel] = None, **kwargs) -> Union[str, BaseModel]:
        ...
        try:
            ...
            return await loop.run_in_executor(None, self.generate, prompt, schema, **kwargs)
        except Exception as e:
    ...
    def _generate_with_schema_response(self, response_text: str, schema: BaseModel) -> BaseModel:
...
class GroqWrapper(DeepEvalBaseLLM):
    ...
    def generate(self, prompt: str, schema: Optional[BaseModel] = None, **kwargs) -> Union[str, BaseModel]:
        """Return just the text for direct calls or structured output if schema provided"""
        if schema is not None:
            # Special handling for DeepEval's Steps schema
            if schema == Steps:
        ...
        result = self._call_llm(prompt, **kwargs)
        # Extract the text content from the SimpleNamespace object
        response_text = result.output if hasattr(result, 'output') else str(result)
    ...
    def _generate_with_schema(
        ...
        {json.dumps(schema.model_json_schema(), indent=2)}
        """
        response_result = self._call_llm(schema_prompt, **kwargs)
        
        # Extract text from SimpleNamespace if needed
        response_text = response_result.output if hasattr(response_result, 'output') else str(response_result)
